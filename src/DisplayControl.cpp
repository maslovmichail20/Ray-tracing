...
DisplayControl::DisplayControl(
    ...
    canvas = empty2;
    canvas[0][0] = camera[0] + (width / 2)*sin(angle) - 1*cos(angle);
    canvas[0][1] = camera[1] - (width / 2)*cos(angle) - 1*sin(angle);
    canvas[0][2] = -0.5;

    canvas[1][0] = camera[0] - (width / 2)*sin(angle) - 1*cos(angle);
    canvas[1][1] = camera[1] + (width / 2)*cos(angle) - 1*sin(angle);
    canvas[1][2] = 0.5;
